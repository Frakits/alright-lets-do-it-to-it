...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'funky hills':
				dialogue = CoolUtil.coolTextFile(Paths.txt('funky hills/funky hillsDialogue'));
			case 'rings':
				dialogue = CoolUtil.coolTextFile(Paths.txt('rings/ringsDialogue'));
			case 'blackout':
				dialogue = CoolUtil.coolTextFile(Paths.txt('blackout/blackoutDialogue'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			case 'funky hills':
			{
				curStage = 'The Mystical Realm Of The Emerald Slop Zone';
				defaultCamZoom = 0.9;

				var sky = new FlxSprite(-400, -100).loadGraphic(Paths.image("sky", "shared"));
				sky.scrollFactor.set(0.2, 0.2);
				sky.antialiasing = true;
				add(sky);

				var mountains = new FlxSprite(-400).loadGraphic(Paths.image("mountains", "shared"));
				mountains.scrollFactor.set(0.5, 0.5);
				mountains.antialiasing = true;
				add(mountains);

				var somefuckingthing = new FlxSprite(-400).loadGraphic(Paths.image("some fucking thing", "shared"));
				somefuckingthing.scrollFactor.set(0.9, 0.9);
				somefuckingthing.antialiasing = true;
				add(somefuckingthing);

				var platforms = new FlxSprite(-400).loadGraphic(Paths.image("platforms", "shared"));
				platforms.scrollFactor.set(0.7, 0.7);
				platforms.antialiasing = true;
				add(platforms);

				var floor = new FlxSprite(-200).loadGraphic(Paths.image("floor", "shared"));
			...
			case 'rings':
			{
				...
				var sky = new FlxSprite(-400, -100).loadGraphic(Paths.image("sky2", "shared"));
				sky.scrollFactor.set(0.2, 0.2);
				sky.antialiasing = true;
				add(sky);

				var mountains = new FlxSprite(-400).loadGraphic(Paths.image("mountains2", "shared"));
				mountains.scrollFactor.set(0.5, 0.5);
				mountains.antialiasing = true;
				add(mountains);

				var somefuckingthing = new FlxSprite(-400).loadGraphic(Paths.image("some_fucking_thing2", "shared"));
				somefuckingthing.scrollFactor.set(0.9, 0.9);
				somefuckingthing.antialiasing = true;
				add(somefuckingthing);

				var platforms = new FlxSprite(-400).loadGraphic(Paths.image("platforms2", "shared"));
				platforms.scrollFactor.set(0.7, 0.7);
				platforms.antialiasing = true;
				add(platforms);

				var floor = new FlxSprite(-200).loadGraphic(Paths.image("floor2", "shared"));
				floor.scrollFactor.set(1, 1);
				floor.antialiasing = true;
				add(floor);
			}
			case 'blackout':
			{
				...
				var sky = new FlxSprite(-400, -100).loadGraphic(Paths.image("sky3", "shared"));
				sky.scrollFactor.set(0.2, 0.2);
				sky.antialiasing = true;
				add(sky);

				var mountains = new FlxSprite(-400).loadGraphic(Paths.image("mountains3", "shared"));
				mountains.scrollFactor.set(0.5, 0.5);
				mountains.antialiasing = true;
				add(mountains);

				var somefuckingthing = new FlxSprite(-400).loadGraphic(Paths.image("some_fucking_thing3", "shared"));
				somefuckingthing.scrollFactor.set(0.9, 0.9);
				somefuckingthing.antialiasing = true;
				add(somefuckingthing);

				var platforms = new FlxSprite(-400).loadGraphic(Paths.image("platforms3", "shared"));
				platforms.scrollFactor.set(0.7, 0.7);
				platforms.antialiasing = true;
				add(platforms);

				var floor = new FlxSprite(-200).loadGraphic(Paths.image("floor3", "shared"));
				floor.scrollFactor.set(1, 1);
				floor.antialiasing = true;
				add(floor);
			}
		...
		if (true)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'thorns':
					schoolIntro(doof);
				case 'funky hills' | 'rings' | 'blackout':
					schoolIntro(doof);
				default:
					startCountdown();
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong == 'BlackOut')
		{
			var songPositionInSeconds = Conductor.songPosition / 1000;
			dad.y = 100 + Math.sin(songPositionInSeconds * 7) * 50;
			dad.x = 100;
			if ((curBeat > 36 && curBeat < 100) || (curBeat > 164 && curBeat < 227)) {
				dad.y = 100 + Math.sin(songPositionInSeconds * 14) * 100 / 2;
				dad.x = 100 + Math.cos(songPositionInSeconds * 12) * 100;
			}
		}
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						daNote.destroy();
						if (dad.curCharacter == "dark-sonic")
